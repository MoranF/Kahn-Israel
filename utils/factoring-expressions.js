(function() {
    ...
    var factorDiffOfSquares = function(expr, options) {
      ...
      if (options.factorDiffOfSquares === "a^2-b^2=(a-b)^2") {
          ...
          hints.push("<p><code>" + KhanUtil.format(coloredExpr) + "</code></p><p>הביטוי הוא מהצורה <code>" + initialForm +
              "</code> שהיא למעשה הפרש של שני ריבועים. לכן אפשר לכתוב אותה כ-<code>" + factoredForm + "</code></p>");
          var strA = KhanUtil.parseFormat("#a", [KhanUtil.PINK]);
          var strB = KhanUtil.parseFormat("#b", [KhanUtil.BLUE]);
          hints.push("<p>מהם ערכיהם של <code>" + strA + "</code> ו- <code>" + strB + "</code>?</p>");
          ...
          hints.push("<p>השתמשו בערכים שמצאתם עבור <code>" + strA + "</code> ועבור <code>" + strB + "</code> כדי להשלים את פירוק הביטוי לגורמים, <code>" + factoredForm + "</code></p>");
          ...
          hints.push("<p><b>אך כך אנו יכולים לרשום את הביטוי כמכפלה של הגורמים: </b><code>" + KhanUtil.format(coloredFactored) + "</code>");
      }
    ...
    var genHintListFactors = function(factors, occFactors) {
       ...
       if (listFactors.length === 1) {
          return hint + "<p>לאיברים יש גורם אחד משותף: " + strListFactors + ".</p>";
       } else {
          var gcf = KhanUtil.format(KhanUtil.exprSetStyle(KhanUtil.genExprFromExpFactors(factors, occFactors), KhanUtil.BLUE));
          return hint + "<p>לאיברים יש שני גורמים משותפים: " + strListFactors + ", לכן הגורם המשותף הגדול ביותר הוא <code>" + gcf + "</code>.</p>";
       }
    };
    ...
    var genHintsDecomposeAllFactors = function(factors, sharedOccFactors, termsOccFactors) {
        ...
        hints.push("<p><code>" + KhanUtil.format(expr) + "</code></p><p>אנו מתחילים בלכתוב כל איבר במכפלה של גורמים הכי פשוטים שלו.</p>");
        ...
        hints.push("<p>ניתן לכתוב את הביטוי בתור: <code>" + KhanUtil.format({op: "+", args: genAllTermsMarkShared(factors, sharedOccFactors, termsOccFactors, colors)}) + "</code>.</p>");
        hints.push("<p>כעת ניתן לכתוב את הביטוי בתור המכפלה הבאה: <code>" + KhanUtil.format(genFullExpr(factors, sharedOccFactors, termsOccFactors, true)) + "</code>.</p>");
        return hints;
    };
    ...
    var solveFactoringExercise = function(expr, options) {
       ...
       var hints = ["<p>כדי לפרק את הביטוי לגורמים, אני מסתכלים תחילה על האיברים ובודקים האם ניתן להוציא מהם גורם משותף. את הגורם המשותף נוציא מחוץ לסוגריים.</p>"];
       ...
       if (options.factorDiffOfSquares) {
          var hint = "<p>אנחנו מקבלים סך הכל את הביטוי הבא: " + KhanUtil.getSubHints("common-factors", "הצג הסבר", detailedHints);
          hint += "<p><code>" + KhanUtil.format(solution) + "</code></p>";
          hints.push(hint);
          hints.push("<p>האם ניתן לפרק את הביטוי לגורמים יותר פשוטים?</p>");
          for (var iArg = 0; iArg < solution.args.length; iArg++) {
             ...
             hints.push("<p>את החלק הזה של הביטוי ניתן לפרק: <code>" + KhanUtil.format(arg) + "</code></p>");
             var hint = "<p>אנו משתמשים בצורה הידועה של הפרש ריבועים כדי לקבל : " + KhanUtil.getSubHints("diff-squares-" + iArg, "הצג הסבר", solvedArg.hints);
       ...
       hints.push("<p class='final_answer'>לא ניתן לפרק לגורמים פשוטים יותר. התשובה הסופית היא : <code>" + KhanUtil.format(solution) + "</code></p>");
