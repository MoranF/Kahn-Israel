function Adder( a, b, digitsA, digitsB ) {
	...
	this.showSideLabel1 = function( str ) {
		highlights.push( graph.label( [ 0, pos.sideY-3 ], str, "right" ) );
	};
	...
	this.showDecimals = function( deciA, deciB ) {
		...
		this.showSideLabel1( "\\text{תחילה ודאו שהנקודות העשרוניות מסודרות אחת מעל השניה}" );
		}
	}
}
...
function Multiplier( a, b, digitsA, digitsB, deciA, deciB ) {
	...
	this.showDecimalsInProduct = function() {
		...
		graph.label( [ x, y + 2 ],
			"\\text{למספר העליון יש " + KhanUtil.plural_heb( deciA, "ספרה","ספרות","f" ) + " מימין לנקודה העשרונית.}", "right" );
		graph.label( [ x,  y + 1 ],
			"\\text{למספר התחתון יש " + KhanUtil.plural_heb( deciB, "ספרה","ספרות","f"  ) + " מימין לנקודה העשרונית.}", "right" );
		graph.label( [ x,  y ],
			"\\text{למכפלה יש " + deciA + " + " + deciB + " = " + ( deciA + deciB )
			 + " מימין לנקודה העשרונית.}", "right" );
		graph.style({
			fill: "#000"
		}, function() {
...
function Divider( divisor, dividend, deciDivisor, deciDividend ) {
    ...
    this.addDecimalRemainder = function() {
        ...
        graph.label([digitsDividend.length, 1],
                "\\text{הוסיפו למונה המקורי נקודה עשרונית ואפס והמשיכו לחלק}", "right");
    };
	this.getNumHints = function() {
	...
	this.shiftDecimals = function() {
		...
		if ( deciDivisor !== 0 ) {
			graph.label( [ digitsDividend.length + 1 + ( deciDiff > 0 ? deciDiff : 0 ), 1 ],
				"\\text{נזיז את הנקודה העשרונית " + deciDivisor + " מקומות לימין.}", "right" );
			graph.style({
				fill: "#000"
			}, function() {
            ...
            graph.label([digitsDividend.length + 0.5, 1.2],
                "\\text{נביא את הנקודה העשרונית לתוך}", "right");
            graph.label([digitsDividend.length + 0.5, 0.8],
                "\\text{התשובה (המנה).}", "right");
		}
