$.extend(KhanUtil, {
    ...
    cardinal: function(n) {
		...
		var cardinalUnits = ["אפס", "אחד", "שתיים", "שלוש", "ארבע", "חמש", "שש", "שבע", "שמונה", "תשע", "עשר", "אחת עשרה", "שתים עשרה", "שלוש עשרה", "ארבע עשרה", "חמש עשרה", "שש עשרה", "שבע עשרה", "שמונה עשרה", "תשע עשרה"];
		var cardinalTens = ["", "", "עשרים", "שלושים", "ארבעים", "חמישים", "שישים", "שבעים", "שמונים", "תשעים"];
        ...
        var smallNumberWords = function(n) {
            ...
            if (hundredDigit) {
                str += cardinalUnits[hundredDigit] + " מאות";
            }
    ...
    scientific: function(precision, num) {
  	...
  	cardinalHeb: function( n ) {   
  		var strHeb = KhanUtil.InCardHeb( n, true, false, "" );
  		return strHeb;
  	},
  
  	InCardHeb: function( n, efes, vav, strAll ) {
  		var hadSif = ["אפס", "אחת", "שתיים", "שלוש", "ארבע", "חמש", "שש", "שבע ", "שמונה", "תשע"];  
  		var duSif = ["אין", "עשר", "עשרים", "שלושים", "ארבעים", "חמישים", "שישים", "שבעים", "שמונים", "תשעים"];  
  		var fourSif = ["אין", "אין", "אין", "שלושת", "ארבעת", "חמשת", "ששת", "שבעת ", "שמונת", "תשעת"];  
  		var str = strAll;  
        
  		if ( n < 10 && n > 0 || (efes && n == 0) ) {
  			str += (vav ? "ו" : "") + hadSif[ n ]; 
  		} else if ( n == 10 ) {
  			str += (vav ? "ו" : "") + duSif[ 1 ];
  		} else if ( n >= 10 && n < 20 ) {
  			str += hadSif[ (n % 10) ] + " עשרה";
  		} else if ( n >= 20 && n < 100 ) {
  			if ( n % 10 == 0 ) {
  				str += duSif[ Math.floor(n / 10) ];
  			} else {
  				str += duSif[ Math.floor(n / 10) ] + " ו" + hadSif[ (n % 10) ];
  			}
  		} else if ( n >= 100 && n < 1000 ) {
  			if ( Math.floor(n / 100) == 1 ) {
  				str += "מאה " + KhanUtil.InCardHeb( n % 100, false, true, str );
  			} else if ( Math.floor(n / 100) == 2 ) {
  				str += "מאתיים " + KhanUtil.InCardHeb( n % 100, false, true, str );
  			} else {
  				str += hadSif[ Math.floor(n / 100) ] + " מאות " + KhanUtil.InCardHeb( n % 100, false, true, str );
  			}
  		} else if ( n >= 1000 && n < 11000 ) {
  			if ( Math.floor(n / 1000) == 1 ) {
  				str += " אלף " + KhanUtil.InCardHeb( n % 1000, false, true, str );
  			} else if ( Math.floor(n / 1000) == 2 ) {
  				str += " אלפיים " + KhanUtil.InCardHeb( n % 1000, false, true, str );
  			} else { 
  				str += fourSif[  Math.floor(n / 1000) ] + " אלפים " + KhanUtil.InCardHeb( n % 1000, false, true, str );
  			}
  		} 
  		return str;   
  	}
