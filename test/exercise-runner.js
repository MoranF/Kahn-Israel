...
$.extend(Khan, {
	testExercise: function( json ) {
		...
		module( exerciseName, {
			setup: function() {
				if ( typeof iframe === "undefined" ) {
					...
					stop();
					$( document ).one( "problemLoaded", function( e, mp ) {
						makeProblem = mp;
						start();
					} );
		...
		$.each( json.problems, function( index, problem, next ) {
			asyncTest( problem.type + " / " + problem.seed, function() {
				...
				$( document ).one( "problemLoaded", function( e, mp, solution ) {
					var VARS = iwindow.$.tmpl.VARS;
